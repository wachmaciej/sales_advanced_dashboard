...
from utils import format_currency_int, format_currency
...
def get_applicable_price_range(channel, df_row):
    """Get the applicable price range for a given channel with flexible matching."""
    if pd.isna(channel):
        return "N/A"
    ...
    if channel in UK_CHANNELS or any(keyword in channel_lower for keyword in ['amazon uk', 'amazon.uk', 'amazon-uk', 'website uk', 'website.uk', 'website-uk']):
        return df_row.get(PRICE_RANGE_UK_COL, 'N/A')
    # Check for US channels (exact match first, then flexible)
    elif channel in US_CHANNELS or any(keyword in channel_lower for keyword in ['amazon us', 'amazon.us', 'amazon-us', 'website us', 'website.us', 'website-us']):
        return df_row.get(PRICE_RANGE_US_COL, 'N/A')
    else:
        return "N/A"  # Default for other channels

# --- Main Display Function ---
def display_tab(df, available_years, default_years):
    """Displays the Category Summary tab using custom category definitions with AOV and price range insights."""
    st.markdown("### 📂 Sales Summary by Category")
    st.markdown("Analyze product performance with AOV and price range insights to identify best/poor performers from price reductions.")
    ...
    # --- Assign Custom Categories and Price Ranges ---
    ...
    # Add applicable price range information
    def get_price_range_for_row(row):
        return get_applicable_price_range(row.get(SALES_CHANNEL_COL), row)
    
    filtered_df['Applicable_Price_Range'] = filtered_df.apply(get_price_range_for_row, axis=1)
    ...
    for category in categories_to_show:
        ...
        agg_category = df_category.groupby(PRODUCT_COL).agg(
            ...
            # Get the most common price range for this product
            Price_Range=('Applicable_Price_Range', lambda x: x.mode().iloc[0] if not x.mode().empty else 'N/A')
        ).reset_index()
        
        # Calculate AOV
        agg_category['AOV'] = agg_category['Item_Revenue'] / agg_category['Items_Purchased']
        agg_category['AOV'] = agg_category['AOV'].fillna(0)
        ...
        grand_total_aov = grand_total_revenue / grand_total_units if grand_total_units > 0 else 0
        ...
        display_df["AOV"] = pd.to_numeric(display_df["AOV"], errors='coerce').apply(format_currency)
        
        # Color code price ranges for easy identification
        def color_price_range(val):
            if val == "Full Price Range":
                return "🟢 " + val
            elif val == "Reduced Range":
                return "🟡 " + val
            elif val == "Stragglers":
                return "🔴 " + val
            elif val == "Zero Stock":
        ...
        display_df["Price Range"] = display_df["Price Range"].apply(color_price_range)
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "AOV": st.column_config.TextColumn("AOV", help="Average Order Value - Revenue per item purchased", width="small"),
                "Price Range": st.column_config.TextColumn("Price Range", help="Most common price range for this product", width="medium")
            }
        )
        ...
        with col1:
            st.markdown(
                ...
                f"AOV: {format_currency(grand_total_aov)}"
            )
        
        with col2:
            # Price reduction insights
            reduced_items = agg_category[agg_category["Price Range"].str.contains("🟡|🔴", na=False)]
            if not reduced_items.empty:
                reduced_count = len(reduced_items)
                total_count = len(agg_category)
                reduced_pct = (reduced_count / total_count * 100) if total_count > 0 else 0
                st.markdown(f"**Price Reductions:** {reduced_count}/{total_count} items ({reduced_pct:.1f}%)")
