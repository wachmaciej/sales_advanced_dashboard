...
def create_yoy_trends_chart(data, selected_years,
    ...
    # Create the line chart with better color scheme
    ...
    # Apply better colors for readability - especially for 2025
    year_color_map = {
        2023: "#6366f1",  # Indigo - good contrast
        2024: "#059669",  # Emerald - strong green
        2025: "#dc2626",  # Red - strong, highly readable
        2026: "#ea580c",  # Orange - if future years exist
        2027: "#7c3aed"   # Purple - if future years exist
    }
    
    # Apply custom colors to each trace based on the year
    for trace in fig.data:
        year_name = trace.name
        try:
            year_int = int(year_name)
            if year_int in year_color_map:
                trace.line.color = year_color_map[year_int]
                trace.marker.color = year_color_map[year_int]
        except (ValueError, TypeError):
            pass  # Keep default color if year parsing fails
...
def create_daily_price_chart(data, listing, selected_years, selected_quarters, selected_channels, week_range=None):
    ...
    # Determine display currency and symbol based on channel selection
    ...
    if ORIGINAL_CURRENCY_COL in df_listing.columns and not df_listing[ORIGINAL_CURRENCY_COL].dropna().empty:
         ...
         if len(unique_currencies) > 0:
             ...
             if not selected_channels or len(selected_channels) == 0 or len(selected_channels) > 1:
                 # No specific channel selected or multiple channels - prioritize GBP
                 if 'GBP' in unique_currencies:
                     display_currency = 'GBP'
                     # Filter data to GBP transactions only
                     df_listing = df_listing[df_listing[ORIGINAL_CURRENCY_COL] == 'GBP']
                 else:
                     # Fallback to USD if GBP not available, then first currency
                     display_currency = 'USD' if 'USD' in unique_currencies else unique_currencies[0]
                     df_listing = df_listing[df_listing[ORIGINAL_CURRENCY_COL] == display_currency]
             else:
                 # Specific single channel selected - use the currency from that channel's transactions
                 channel_currencies = df_listing[ORIGINAL_CURRENCY_COL].dropna().unique()
                 if len(channel_currencies) == 1:
                     display_currency = channel_currencies[0]
                 elif 'GBP' in channel_currencies:
                     # If multiple currencies in selected channel, still prefer GBP
                     display_currency = 'GBP'
                     df_listing = df_listing[df_listing[ORIGINAL_CURRENCY_COL] == 'GBP']
                 else:
                     # Use the first available currency for this channel
                     display_currency = channel_currencies[0]
                     df_listing = df_listing[df_listing[ORIGINAL_CURRENCY_COL] == display_currency]
             
             # Set currency symbol
             currency_map = {"GBP": "£", "USD": "$", "EUR": "€", "CAD": "C$", "AUD": "A$", "MXN": "MX$", "SEK": "SEK"}
             currency_symbol = currency_map.get(display_currency, f"{display_currency} ")
             
             # Show info message about currency selection
             if len(unique_currencies) > 1:
                 if not selected_channels or len(selected_channels) == 0 or len(selected_channels) > 1:
                     st.info(f"📊 Multiple currencies available ({sorted(unique_currencies)}). Displaying prices in **{display_currency}** (prioritized). Use channel filters to see specific currency data.")
                 else:
                     st.info(f"💰 Displaying prices for '{selected_channels[0]}' channel in **{display_currency}**. Available currencies: {sorted(unique_currencies)}")

    # Check if we still have data after currency filtering
    if df_listing.empty:
        st.warning(f"No data available for '{listing}' in the selected currency after filtering.")
        return go.Figure().update_layout(title_text=f"No {display_currency} data for '{listing}'")
    ...
    for yr in selected_years_int:
        ...
        # Determine the end day for reindexing
        start_day = int(df_year["Day"].min())
        
        # For current year, only go up to today's day of year
        # For past years, use the full range of actual data
        if yr == current_year:
            # Only include data up to today for current year
            end_day = min(int(df_year["Day"].max()), current_day_of_year)
            # Filter out any future data points
            df_year = df_year[df_year["Day"] <= current_day_of_year]
        else:
            # For past years, use all available data
            end_day = int(df_year["Day"].max())
        
        # Skip if no valid data remains after filtering
        if df_year.empty or start_day > end_day:
            continue
            
        # Reindex to fill missing days within the appropriate range
    ...
    # Create the final line chart with custom color mapping
    # Determine current year for color assignment
    current_year = datetime.datetime.now().year
    
    # Create custom color mapping: red for current year, blue for previous years
    unique_years = sorted(combined[year_col_to_use].unique())
    color_discrete_map = {}
    
    for year in unique_years:
        if year == current_year:
            color_discrete_map[year] = "#ef4444"  # Red for current year
        else:
            color_discrete_map[year] = "#3b82f6"  # Blue for previous years
    ...
    fig = px.line(
        ...
        color_discrete_map=color_discrete_map # Use custom color mapping
    )

    # Layout adjustments
    fig.update_layout(
        title=f"📈 Daily Average Price Trend - {listing}",
        margin=dict(t=50, b=50),
        legend_title_text='Year',
        hovermode='x unified'
    )
    ...
    fig.update_traces(line_width=2, marker_size=3)
